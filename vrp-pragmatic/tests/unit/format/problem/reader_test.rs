...
use crate::constraints::{JobSkills as ConstraintJobSkills, SKILLS_DIMEN_KEY};
...
fn assert_job_skills(dimens: &Dimensions, expected: Option<Vec<String>>) {
    let skills = dimens.get_value::<ConstraintJobSkills>(SKILLS_DIMEN_KEY);
...
fn assert_vehicle_skills(dimens: &Dimensions, expected: Option<Vec<String>>) {
    let skills = dimens.get_value::<HashSet<String>>(SKILLS_DIMEN_KEY);
    if let Some(expected) = expected {
        let expected = HashSet::from_iter(expected.iter().cloned());
